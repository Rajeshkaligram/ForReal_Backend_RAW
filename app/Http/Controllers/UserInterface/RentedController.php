...
//        return $this->data['rented'];
    ...
    public function payment_success(Request $req, $id) {
        ...
        $paymentId = (string) $req->input('paymentId', '');
        ...
        if ($rented_detail) {
            ...
            $RentTransactD->detail           = "Client payment marked accepted (legacy PayPal REST callback removed)";
    ...
    public function payRental(Request $req) {
        $rented_id = $req->id;
        Helper::flashMessage('Error!', 'Legacy PayPal REST checkout removed. Use the supported checkout flow.', 'error');
        return redirect('rented/view/' . $rented_id);
    }
    ...
    public function refundToClient($rented_id, $refundAmount, $user_id, $detail) {
        ...
        $RentTransactD->payment_info     = 'Refund logged only (legacy PayPal REST removed)';
        $RentTransactD->save();
    }
    ...
    public function payToMerchant($email, $rent, $rented_detail, $detail) {
        ...
        $RentTransactD->detail           = $detail;
        $RentTransactD->pay_key          = '';
        $RentTransactD->payment_info     = 'Merchant payout logged only (legacy PayPal REST removed)';
        $RentTransactD->save();
    }
