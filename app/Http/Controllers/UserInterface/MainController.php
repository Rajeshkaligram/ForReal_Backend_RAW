    ...
    function postRegistration(Request $request)
    {
        ...
        if ($validator === true) {
            ...
            if ($request->has('id')) {
                ...
                $data['url'] = URL('verify-registration/' . urlencode(Crypt::encrypt($user->email)));
...
//                $user->notify(new Registration($user,$this->data['configuration']));
    ...
    function getVerifyRegistration($token)
    {
        if (empty($token) || $token === 'null') {
            Helper::flashMessage('Action failed', 'Invalid verification link.', 'error');
            return redirect('/');
        }

        try {
            $email = Crypt::decrypt(urldecode($token));
        } catch (DecryptException $e) {
            Helper::flashMessage('Action failed', 'Invalid or expired verification link.', 'error');
            return redirect('/');
        }
        ...
        if ($validator === true) {
            if (Auth::attempt(['email' => $request->email, 'password' => $request->password])) {
                if (Auth::user()->status == 0) {
                    ...
                    $data['title']   = 'Good day!';
                    $data['message'] = 'Thank you for registering with '. $this->data['configuration']->name . '!. <br><br> Click the button to complete your registration.';
                    $data['url'] = URL('verify-registration/' . urlencode(Crypt::encrypt(Auth::user()->email)));
                    $data['button'] = "Verify Registration";
                    $data['name'] = $this->data['configuration']->name;
...
XML;
    ...
    public function ups_usa($request) {
        $result = app(UpsRateService::class)->quote([
            'from_address' => clean($request->from_address),
            'from_city' => $request->from_city,
            'from_state_province_code' => $request->from_state_province_code,
            'from_countries' => $request->from_countries,
            'from_zipcode' => $request->from_zipcode,
            'destination_address' => clean($request->destination_address),
            'destination_city' => $request->destination_city,
            'destination_state_province_code' => $request->destination_state_province_code,
        ...
        if (!$result['success']) {
            $error['some_error'][0] = $result['message'];
            return response()->json(["result" => 'failed', "errors" => $error]);
        }

        $plan = $result['shipping_plans'][0];
        $result_string = "<p>Shipping Charges <span class='pull-right'>".$plan['cost']."</span></p>";
        $result_string .= "<h1>Note: This item will be released and delivered on or within 10 days</h1>";
        return response()->json(["result" => 'success', 'data' => $result_string]);
    }
